...
public class ParticleGenerator : MonoBehaviour {		
	...
	public GameObject particlesParent; // Where will the spawned particles will be parented (To avoid covering the whole inspector with them)
	//public Sprite _srite;

	void Start() {
		particlesParent = GameObject.Find("ParticlesParent");
	}
	...
	void Update() {	
		if( lastSpawnTime+SPAWN_INTERVAL<Time.time ){ // Is it time already for spawning a new particle?
			...
			newLiquidParticle.transform.parent=particlesParent.transform;// Add the particle to the parent container			
			lastSpawnTime=Time.time; // Register the last spawnTime

			newLiquidParticle.GetComponent<Renderer>().sortingLayerName = "Liquids";
			//newLiquidParticle.GetComponent<SpriteRenderer>().sprite = _srite;
		}		
	}
}
